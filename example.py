...
from errprop import ErrorPropagation, error_symbols, set_significant_digits, dict_latex
...
err_variables = ml, mg, mRs, mR, malpha, mb, md = error_symbols(l, g, Rs, R, alpha, b, d)
...
errors = {ml:0.01, mg:0.0, mRs:0.001, mR:0.00005, malpha:0.0, mb:0.00001, md:0.00001}
...
ep.print_all(errors, values1, "G_{Al}", n_split=4)

print("Schubmodul Messing")
ep = ErrorPropagation(formula, l, g, Rs, R, alpha)
ep.print_all(errors, values2, "G_{Ms}", n_split=4)

print("Schubmodul Eisen")
ep = ErrorPropagation(formula, l, g, Rs, R, alpha)
ep.print_all(errors,    values3, "G_{Fe}", n_split=4)

print("Elastizitätsmodul Stahl (Stab 3)")
ep = ErrorPropagation(formula2, l, g, alpha, b, d)
ep.print_all(errors, values4, "E_{steel}", n_split=4)

print("Elastizitätsmodul Al (Stab 6)")
ep = ErrorPropagation(formula2, l, g, alpha, b, d)
ep.print_all(errors, values5, "E_{aluminium}", n_split=4)

print("Elastizitätsmodul Messing (Stab 7)")
ep = ErrorPropagation(formula2, l, g, alpha, b, d)
ep.print_all(errors, values6, "E_{brass}", n_split=4)

##a = ep.formula_to_latex(symbol = "E_{brass}", align=False)
##b = ep.formula_to_latex(symbol = "E_{brass}", align=True)
##c = ep.formula_to_latex(symbol = "E_{brass}", align=True, values = values6)
##
##print(a)
##print()
##print(b)
...
##print()
##
##print(ep.error_to_latex(symbol = "EEE", align = False, values = values6, errors = errors))
##print(ep.error_to_latex(symbol = "EEE", align = False))
